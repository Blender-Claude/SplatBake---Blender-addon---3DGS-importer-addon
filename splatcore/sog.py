...
IMAGE_READER = None
"""Host-supplied image decoder, set by whoever embeds this package.

SOG stores its data as PNG/WebP images, so something has to decode them -
and that decoder was the one and only reason this module ever imported bpy.
Blender's loader handles WebP with no extra dependency, which is why the
add-on installs it here at registration; a standalone user can install any
callable, and Pillow is used automatically if nothing is set. Keeping the
choice OUT of this module is what lets the package stand alone under Apache
2.0 while the Blender half stays GPL.

Signature: reader(path) -> (H, W, 4) uint8 array, origin TOP-LEFT.
"""


def read_u8_image(path):
    """Decode an image to a (H, W, 4) uint8 array with the origin at the
    TOP-LEFT. Uses IMAGE_READER when the host has installed one, else
    Pillow."""
    if IMAGE_READER is not None:
        return IMAGE_READER(path)
    try:
        from PIL import Image
    except ImportError:
        raise RuntimeError(
            "no image decoder available: set sog.IMAGE_READER to a "
            "callable(path) -> (H, W, 4) uint8 array, or install Pillow"
        )
